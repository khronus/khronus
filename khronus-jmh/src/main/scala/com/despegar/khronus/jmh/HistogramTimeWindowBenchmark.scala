...
class HistogramTimeWindowBenchmark {
  ...
  val timeWindow30s = new HistogramTimeWindow(30 seconds, 1 millis) {
    override val bucketStore: BucketStore[HistogramBucket] = new BucketStore[HistogramBucket] {
      ...
      override def store(metric: Metric, windowDuration: Duration, buckets: Seq[HistogramBucket]): Future[Unit] = Future.successful {}
    ...
    override val metaStore: MetaStore = new MetaStore {
  ...
  def timeWindow30Seconds() = {
    Await.ready(timeWindow30s.process(metric, tick), 5 seconds)
  }
