...
class KhronusHandler extends HttpServiceActor with KhronusHandlerException {
...
trait KhronusHandlerException extends Logging {
  implicit def khronusExceptionHandler(implicit settings: RoutingSettings) =
    ExceptionHandler {
      case exception: UnsupportedOperationException ⇒ ctx ⇒
        ...
        log.error(s"Handling Exception", exception)
