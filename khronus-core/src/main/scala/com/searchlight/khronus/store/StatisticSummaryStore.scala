...
trait StatisticSummarySupport extends SummaryStoreSupport[HistogramSummary] {
...
class CassandraHistogramSummaryStore(session: Session) extends CassandraSummaryStore[HistogramSummary](session) with Logging with Measurable {
  ...
  override def deserialize(timestamp: Long, buffer: Array[Byte]): HistogramSummary = {
    ...
    version match {
      ...
      case _ ⇒ throw new UnsupportedOperationException(s"Could not deserialize unknown HistogramSummary version $version")
    }
  }
...
object HistogramSummarySerializerV1 extends StatisticSummarySerializer {
...
object HistogramSummarySerializerV2 extends StatisticSummarySerializer {
