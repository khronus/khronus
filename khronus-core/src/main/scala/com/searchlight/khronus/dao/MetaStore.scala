...
case class MetricMetadataDetailed(metric: Metric, lastProcessedTimestamp: Timestamp, active: Boolean)

case class MetricMetadata(name: String, @JsonProperty("type") mtype: MetricType, tags: Map[String, Seq[String]] = Map())
...
trait MetaStore extends Snapshot[Map[Metric, (Timestamp, Boolean)]] {
  ...
  def getMetricByName(metricName: String): Metric

  def searchMetrics(regex: String): Seq[MetricMetadata]
...
class CassandraMetaStore(session: Session) extends MetaStore with Logging with CassandraUtils with ConcurrencySupport with MonitoringSupport with Measurable {
  ...
  def searchMetrics(regex: String): Seq[MetricMetadata] = {
    ...
    snapshot.keys.filter(k ⇒ matcher.reset(k.name).matches()).groupBy(_.name).values.toSeq.map { metrics ⇒
      ...
      MetricMetadata(head.name, head.mtype, metrics.toSeq.flatMap(_.tags.toSeq).groupBy(_._1).mapValues(seq ⇒ seq.map(_._2)))
    }
  }

  def getMetricByName(metricName: String) = {
    searchInSnapshotByMetricName(metricName).map(_._1).get
  }
  ...
  private def toMetric(key: String): Metric = {
    ...
    if (tokens.length > 2) {
      val flatName = (key splitAt (key lastIndexOf '|'))._1
      Metric.fromFlatNameToMetric(flatName, tokens.last)
    } else {
