...
trait SummaryStore[T <: Summary] {
  ...
  def readAll(metric: String, windowDuration: Duration, slice: TimeRange, ascendingOrder: Boolean = true, count: Int): Future[Seq[T]]
}
...
abstract class CassandraSummaryStore[T <: Summary](session: Session) extends SummaryStore[T] with Logging with Measurable with ConcurrencySupport with CassandraUtils {
  ...
  def sliceUntilNow(metric: Metric, windowDuration: Duration): Future[Seq[T]] = {
    val slice = TimeRange(to = now)
    readAll(metric.flatName, windowDuration, slice)
  }

  def readAll(metric: String, windowDuration: Duration, slice: TimeRange, ascendingOrder: Boolean = true, count: Int = limit): Future[Seq[T]] = measureFutureTime("summary.readAll", "summary.readAll") {
