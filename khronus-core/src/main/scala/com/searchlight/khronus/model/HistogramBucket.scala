...
class HistogramBucket(override val bucketNumber: BucketNumber, val histogram: Histogram) extends Bucket(bucketNumber) with Logging {
  ...
  override def summary: HistogramSummary = Try {
    ...
    case e: Exception ⇒
      ...
      printStream.flush()
      log.error(s"Failure creating HistogramSummary: totalCount: ${histogram.getTotalCount}, percentileDistribution: ${baos.toString}", e)
      Failure(e)
  }.get
}
...
object HistogramBucket extends Measurable {
  ...
  private val tenHoursInMillis = 1000L * 60 * 60 * 10 //10 hours in millis

  private val defaultMaxValue = tenHoursInMillis //10hours in millis, 36seconds in micros, 35mb in bytes, 35gb in kb, 35tb in mb
  private val defaultPrecision = 3
  ...
  def newHistogram = SkinnyHistogram(defaultMaxValue, defaultPrecision)
}