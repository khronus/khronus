...
class SkinnyHistogramTest extends FunSuite with MockitoSugar with Matchers with Logging {
  ...
  test("serialization round-trip") {
    val skinnyHistogram = new SkinnyHistogram(3600000000L, 3)
    ...
    val bytesEncoded = skinnyHistogram.encodeIntoByteBuffer(buffer)
    buffer.limit(bytesEncoded)
    buffer.rewind()

    val decodedHistogram = SkinnyHistogram.decodeFromByteBuffer(buffer)

    skinnyHistogram shouldEqual decodedHistogram
  }
  ...
  test("compression round-trip") {
    ...
    val decodedHistogram = SkinnyHistogram.decodeFromCompressedByteBuffer(buffer, 0L)
  ...
  test("should compress better than hdrhistogram") {
    val skinnyHistogram = new SkinnyHistogram(3600000000L, 3)
    val hdrHistogram = new Histogram(3600000000L, 3)
    ...
    val skinnyBytes = skinnyHistogram.encodeIntoCompressedByteBuffer(ByteBuffer.allocate(skinnyHistogram.getEstimatedFootprintInBytes))
  ...
  test("should serialize better than hdrhistogram") {
    ...
    val skinnyBytes = skinnyHistogram.encodeIntoByteBuffer(ByteBuffer.allocate(skinnyHistogram.getEstimatedFootprintInBytes))
  ...
  test("should deserialize hdrHistograms to maintain backward compatibility") {
    ...
    val buffer = ByteBuffer.allocate(hdrHistogram.getEstimatedFootprintInBytes)
    val hdrBytes = hdrHistogram.encodeIntoCompressedByteBuffer(buffer)
    buffer.limit(hdrBytes)
    buffer.rewind()

    val histogram = SkinnyHistogram.decodeFromCompressedByteBuffer(buffer, 0L)

    hdrHistogram shouldEqual histogram
  }

  test("some realworld latencies") {
    ...
    val skinnyHistogram = new SkinnyHistogram(3600000000L, 2)
    val hdrHistogram = new Histogram(3600000000L, 2)
    ...
    val skinnyBytesCompressed = skinnyHistogram.encodeIntoCompressedByteBuffer(ByteBuffer.allocate(skinnyHistogram.getEstimatedFootprintInBytes))
    val hdrBytesCompressed = hdrHistogram.encodeIntoCompressedByteBuffer(ByteBuffer.allocate(hdrHistogram.getEstimatedFootprintInBytes))
