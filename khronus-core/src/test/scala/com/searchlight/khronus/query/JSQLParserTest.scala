...
class JSQLParserTest extends Test {
  ...
  test("parse sql with two metrics") {
    ...
    val query = parser.parse(sql)
    query should equal(DynamicQuery(Seq(Count("m1"), Count("m2")), Seq(QMetric("metric1", "m1"), QMetric("metric2", "m2")), Some(And(Equals("m1", "tag1", "23"), Equals("m2", "tag1", "20"))), TimeRange(1, 10)))
  }

  test("parse sql div") {
    val sql = "select count(m1) / count(m2) from metric1 m1, metric2 m2 where time > 1 and time < 10 and m1.tag1 = 23 and m2.tag1 = 20 "
    val query = parser.parse(sql)
    query should equal(DynamicQuery(Seq(DivProjection(Count("m1"), Count("m2"))), Seq(QMetric("metric1", "m1"), QMetric("metric2", "m2")), Some(And(Equals("m1", "tag1", "23"), Equals("m2", "tag1", "20"))), TimeRange(1, 10)))
  }
