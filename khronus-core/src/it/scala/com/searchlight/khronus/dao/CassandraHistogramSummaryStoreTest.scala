...
class CassandraHistogramSummaryStoreTest extends FunSuite with BaseIntegrationTest with Matchers{
  ...
  test("Read should return only the right summary") {
    ...
    val bucketsFromCassandra = await { Summaries.histogramSummaryStore.readAll("testMetric", 30 seconds, TimeRange(1500, 2500)) }
  ...
  test("Reading with descending order should returns timestamp desc") {
    ...
    val bucketsFromCassandra = await { Summaries.histogramSummaryStore.readAll("testMetric", 30 seconds, TimeRange(0, 100), ASCENDING_ORDER) }
