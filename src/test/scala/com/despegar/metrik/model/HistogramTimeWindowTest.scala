...
class HistogramTimeWindowTest extends FunSuite with MockitoSugar {
  ...
  test("with previous buckets should store its buckets and summaries and remove previous buckets") {
    ...
    val executionTime = previousBucket3.timestamp.toBucketNumber(30 seconds).toTimestamp() //The last one

    val histogramA: Histogram = Seq(previousBucket1, previousBucket2)
    val histogramB: Histogram = Seq(previousBucket3)

    val bucketA = new HistogramBucket((1, windowDuration), histogramA)
    val bucketB = new HistogramBucket((2, windowDuration), histogramB)
    val myBuckets = Seq(bucketB, bucketA)

    val summaryBucketA = StatisticSummary(30000, 50, 80, 90, 95, 99, 100, 1, 100, 100, 50)
    val summaryBucketB = StatisticSummary(60000, 100, 100, 100, 100, 100, 100, 100, 100, 2, 100)
  ...
  test("1 minute window should increment last bucket") {
    ...
    val window = mockedWindow(windowDuration, previousWindowDuration)
    val executionTime = 1415205230052L + 50000L //The last one

    when(window.metaStore.getLastProcessedTimestamp(metric)).thenReturn(Future[Timestamp](1415205210000L))

    val previousBucket1 = new HistogramBucket((47173506L, previousWindowDuration), histogram1)
  ...
  test("with already processed buckets should remove them without storing any bucket or summary") {
    ...
    val executionTime = somePreviousBucket.timestamp.toBucketNumber(30 seconds).toTimestamp()
  ...
  test("without previous buckets should do nothing") {
    ...
    when(window.bucketStore.sliceUntil(Matchers.eq(metric), Matchers.any[Timestamp], Matchers.eq(previousWindowDuration))).thenReturn(Future(Seq()))
    when(window.bucketStore.store(metric, windowDuration, Seq())).thenReturn(Future {})
    when(window.summaryStore.store(metric, windowDuration, Seq())).thenReturn(Future {})
